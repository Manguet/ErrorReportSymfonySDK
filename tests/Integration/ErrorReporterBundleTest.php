...
class ErrorReporterBundleTest extends TestCase
{
    ...
    public function testServiceConfiguration(): void
    {
        ...
        $this->assertEquals('https://prod.example.com', $arguments['$webhookUrl']);
        $this->assertEquals('prod-token', $arguments['$token']);
        $this->assertEquals('prod-project', $arguments['$projectName']);
        $this->assertFalse($arguments['$enabled']); // enabled = false
        $this->assertEquals(['ProductionException', 'AnotherException'], $arguments['$ignoredExceptions']);
    }

    public function testEventListenerIsTagged(): void
    {
        $container = new ContainerBuilder(new ParameterBag([
            'kernel.environment' => 'test'
        ]));
